...
var comp2 = [];
function AssignIncidentList() {
   var comp = [];
   comp2 = [];
   $.ajax({
      url: baseurl + "HelpDesk/AllIncidentList",
      type: "GET",
      dataType: "json",
      ...
      success: function (res) {
         ...
         for (var i = 0; i < res.length; i++) {
            var str = ['' + (i + 1) + '', '' + res[i].title + '', '' + res[i].deadline + '', '' + res[i].denieddate + '', '' + res[i].status +'' ];
               ...
               { title: 'status' },
               {
                  ...
                  render: function (data, type, full, meta) {
                     return (
                        ...
                        '<a class="dropdown-item" id="btnDetails"  asp-action="AssignIncidentDetail" asp-controller="HelpDesk" asp-route-incidentId="@Model.incidentId">' +
                        ...
                        'Return Incident</a>' +
                        '<a class="dropdown-item close-record" id="btnCloseIncident"  asp-action="CloseIncident" asp-controller="HelpDesk" asp-route-incidentId="@Model.Incidentid">' +
                        feather.icons['trash-2'].toSvg({ class: 'me-50 font-small-4' }) +
                        'Close Incident</a>' +
                        '</div>' +
                        '</div>' +
...
function CloseIncidentList() {
   ...
   comp2 = [];
   $.ajax({
      url: baseurl + "HelpDesk/GetCloseIncidentList",
      type: "GET",
      dataType: "json",
      contentType: 'application/json',
      success: function (res) {
         console.log(res)
         for (var i = 0; i < res.length; i++) {
            var str = ['' + (i + 1) + '', '' + res[i].title + '', '' + res[i].servicedate + '', '' + res[i].status +''];
...
$('body').on('click', '#btnReturn', function () {
   ...
   var dta = comp2[ind];
   $.ajax({
      url: baseurl + "HelpDesk/DeniedIncident",
      type: "GET",
      dataType: "json",
      contentType: "application/json",
      data: { assignid: dta.assignid },
      success: function (res) {
         console.log(res);
...
$('body').on('click', '#btnCloseIncident', function () {
   var dt = $('.datatables-basic').DataTable();
   var row = $(this).parents('tr')[0];
   var sl = dt.row(row).data()[0];
   var ind = comp2.findIndex(f => f.sl == sl);
   var dta = comp2[ind];
   $.ajax({
      url: baseurl + "HelpDesk/CloseIncident",
      type: "GET",
      dataType: "json",
      contentType: "application/json",
      data: { Incidentid: dta.incidentid },
      success: function (res) {
         console.log(res);
...
$('body').on('click', '#btnDetails', function () {
   ...
   var dta = comp2[ind];
   $.ajax({
      url: baseurl + "HelpDesk/AssignIncidentDetail",
      type: "GET",
      dataType: "json",
      contentType: "application/json",
      data: { Incidentid: dta.incidentid },
